...
func main () {
 ...
 //Untuk Menampilkan Interface Inputan
 fmt.Println()
 fmt.Println()
 fmt.Println("******************************************************")
...
// untuk mengalokasikan slot pada Stack
for index != mobil + 1 {
	fmt.Print("Masukkan Jumlah Barang Mobil ke ")
	fmt.Print(index)
	fmt.Print(" : ")
	fmt.Scanln(&jumlah)
	s.Push(jumlah) //Fungsi Simpan
...
//Untuk Menampilkan hasil inputan
 ...
 fmt.Println("Jumlah Barang yang masuk :")
 index = index - 1
 fmt.Println()
 fmt.Println("------------------------------")
 for index >= 1 {
